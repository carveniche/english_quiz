        ...
        deleteFromActiveTab:(state,action:PayloadAction<String>)=>{
            ...
            if(!activeTabArray.length){
               const {path,key,icon,name}=defaultRouter
                activeTabArray=[{
                    path,key,icon,name,extraParams:{}
                }]
                currentTab=path
            }
            else{
                ...
                if(action.payload==state.currentSelectedKey){
                    ...
                    currentSelectedKey=state.activeTabArray[activeTabArray.length-1].key
                }
            ...
            return {...state,activeTabArray,currentSelectedRouter:currentTab,currentSelectedIndex:i,currentSelectedKey}
        }
    }
})
